...
{
    (void) newAccentColor;
    phasewraithProcessor = dynamic_cast<PhaseWraithPlugin*> (&processor);
    ...
    if (phasewraithProcessor != nullptr)
    {
        ...
        {
            phasewraithProcessor->setAuditionEnabled (! phasewraithProcessor->isAuditionEnabled());
        ...
        {
            phasewraithProcessor->setAuditionType (1 - phasewraithProcessor->getAuditionType());
            syncAuditionControls();
        };
...
#if defined(YUP_AUDIO_PLUGIN_ENABLE_STANDALONE)
    if (phasewraithProcessor != nullptr && inputMeter != nullptr && outputMeter != nullptr)
    {
        const auto latestInputPeak = phasewraithProcessor->getInputPeakLevel();
        const auto latestPeak = phasewraithProcessor->getOutputPeakLevel();
...
{
#if defined(YUP_AUDIO_PLUGIN_ENABLE_STANDALONE)
    if (phasewraithProcessor == nullptr || auditionButton == nullptr || auditionTypeButton == nullptr)
        return;

    auditionButton->setButtonText (phasewraithProcessor->isAuditionEnabled() ? "Audition On" : "Audition Off");
    auditionTypeButton->setButtonText (phasewraithProcessor->getAuditionType() == 0 ? "Saw" : "Pulse");
    static_cast<ehl::ui::CommandButton*> (auditionButton.get())->setSelected (phasewraithProcessor->isAuditionEnabled());
    static_cast<ehl::ui::CommandButton*> (auditionTypeButton.get())->setSelected (phasewraithProcessor->getAuditionType() != 0);
#endif
}
